using System;
using System.Collections.Generic;
using System.Linq;
using System.Reflection;
using System.Text;
using System.Threading.Tasks;

namespace AR.Azure.Mapping
{
    public static class AzureMappingFactory
    {
        private static IEnumerable<Type> _mappings = null;

        private static IEnumerable<Type> Mappings
        {
            get
            {
                if (_mappings == null)
                {
                    Type mappingType = typeof(IAzureMapping<>);
                    _mappings = Assembly.GetExecutingAssembly().GetTypes()
                        .Where
                        (
                                    t => !t.IsAbstract
                                && !t.IsInterface
                                && t.GetInterfaces().Any(g => g.IsGenericType && g.GetGenericTypeDefinition() == mappingType)
                        ).ToList();
                }
                return _mappings;
            }
        }

        public static IAzureMapping<T> GetMapping<T>() where T : class
        {
            Type implementationType = typeof(IAzureMapping<T>);
            Type mappingType = Mappings.FirstOrDefault(x => implementationType.IsAssignableFrom(x));

            if (mappingType == null)
            {
                throw new AzureException("Azure mapping not found for {0}", typeof(T));
            }

            IAzureMapping<T> result = Activator.CreateInstance(mappingType) as IAzureMapping<T>;
            if (result == null)
            {
                throw new AzureException("Unable to create instance of {0}", mappingType);
            }

            return result;
        }
    }
}
