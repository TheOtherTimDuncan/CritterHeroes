...
namespace CritterHeroes.Web.Areas.Admin.Lists.QueryHandlers
{
    ...
    {
        public BreedDashboardItemStatusQueryHandler(ISqlStorageContext<Breed> source, IRescueGroupsStorageContext<Breed> target, IStateManager<OrganizationContext> orgStateManager)
        ...
        protected override async Task<DataResult> GetSourceItems(DashboardStatusQuery<Breed> query, IStorageContext<Breed> storageContext)
        {
            ...
            result.Items = result.Items.Where(x => this.OrganizationContext.SupportedCritters.Any(s => s.Name == x.Species));
            return result;
        }
    }
}
